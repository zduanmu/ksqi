...
class WaterlooSQoE3(StreamingDatabase):
    ...
    def _download(self, root_dir, version='feature'):
        ...
        if not os.path.isdir(root_dir):
            ...
            if all([os.path.isfile(os.path.join(root_dir, 'streaming_logs', s))
                ...
                print('WaterlooSQoE-III already downloaded.')
                return
        
        url = 'https://ivc.uwaterloo.ca/database/WaterlooSQoE-III/'
        if version == 'full':
            filename = 'waterloo_sqoe3_full.zip'
            url += filename
        elif version == 'feature':
            filename = 'waterloo_sqoe3_feature.zip'
            url += filename
        else:
            filename = 'waterloo_sqoe3_server_video.zip'
            url += filename

        zip_filename = os.path.join(root_dir, filename)
        if not os.path.isfile(zip_filename):
            print('Downloading WaterlooSQoE-III database...\n')
            download(url, zip_filename)
            print('WaterlooSQoE-III database download complete!\n')
